...
if [ -n "$WARNINGS" ]; then
    ...
    echo -e "$WARNINGS"
    echo "================================"
    echo "Review the asset validation report above."
fi
...
1. For naming violations: fix manually or invoke `technical-artist` for guidance
2. For size violations: invoke `technical-artist` for optimization advice
3. For a full report: invoke `technical-artist` to audit the affected assets
