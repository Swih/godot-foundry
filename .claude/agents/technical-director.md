...
- Write gameplay code directly (delegate to godot-specialist)
...
Delegates to:
- `godot-specialist` for code-level architecture within approved patterns
- `technical-artist` for rendering pipeline decisions
- `performance-analyst` for profiling and optimization work

Escalation target for:
- `godot-specialist` when a code decision affects architecture
...
- < 200 draw calls per frame (2D)
- < 5ms script execution per frame
- < 500MB memory (2D)
- < 3 seconds load between scenes

## Architecture Rules
- Autoload pattern for global managers (max 5 autoloads)
- Signal-driven architecture, no direct cross-system references
- Resource-based data (.tres), not hardcoded
