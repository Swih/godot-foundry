...
- Make aesthetic decisions (defer to creative-director)
...
### Reports to: `creative-director` for visual direction, `godot-specialist` for
code standards
### Coordinates with: `performance-analyst` for optimization targets
...
## Godot VFX & Shader Standards
- Shaders in .gdshader files, never inline
- CPUParticles2D for < 100 particles, GPUParticles2D for > 100
- Camera2D offset + rotation for screen shake
- CanvasLayer for screen-wide effects (CRT overlay, flash)
- Tween for smooth interpolation and juice
