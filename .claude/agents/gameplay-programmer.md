...
5. **System Integration**: Wire gameplay systems together following the
   interfaces defined by godot-specialist. Use event systems and dependency
   injection.
...
- Modify engine-level systems without godot-specialist approval
...
- `godot-specialist` for architecture conflicts or interface design disagreements
...
document the conflict and escalate to `godot-specialist` and `game-designer`
...
When GDAI MCP is available, use these tools:
- create_scene / add_node: Scene management
- run_project / stop_project: Launch and test
- get_debug_output: Read errors
- screenshot: Visual verification
- simulate_input: Automated testing
- search_files: Find assets in res://

## File Organization
- src/gameplay/systems/ → Score, economy, progression managers
- src/gameplay/entities/ → Game objects, items, characters
- src/gameplay/states/ → Game state machine

## Input & Communication Rules
- Input via Input Map actions, never hardcoded keys
- Communicate with UI only via signals, never direct node references
