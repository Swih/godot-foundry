...
# Check that prototypes/ files don't import from src/
PROTO_FILES=$(echo "$STAGED" | grep -E '^prototypes/.*\.gd$')
if [ -n "$PROTO_FILES" ]; then
    while IFS= read -r file; do
        if [ -f "$file" ]; then
            VIOLATIONS=$(grep -nE '(preload|load)\("res://src/' "$file" 2>/dev/null)
            if [ -n "$VIOLATIONS" ]; then
                WARNINGS="$WARNINGS\nBOUNDARY: $file imports from src/. Prototypes must not depend on src/."
...
# Check for magic numbers in src/gameplay/ .gd files
GAMEPLAY_GD_FILES=$(echo "$STAGED" | grep -E '^src/gameplay/.*\.gd$')
if [ -n "$GAMEPLAY_GD_FILES" ]; then
    while IFS= read -r file; do
        if [ -f "$file" ]; then
            # Find lines with bare numbers > 1 that aren't const, @export, enum, or comments
            MAGIC=$(grep -nE '[^a-zA-Z_][0-9]{2,}|[^a-zA-Z_][2-9]' "$file" 2>/dev/null \
                | grep -vE '(const |@export|enum |#)' 2>/dev/null)
            if [ -n "$MAGIC" ]; then
                WARNINGS="$WARNINGS\nMAGIC_NUM: $file may contain magic numbers. Use constants or @export vars."
            fi
        fi
    done <<< "$GAMEPLAY_GD_FILES"
fi
