...
2. Based on their answers, offer two paths:
   ...
   - **Jump to implementation**: If they're confident in their concept, go
     straight to writing the GDD and prototyping afterward
3. Show the recommended path (adapted to their choice):
   - `/brainstorm` (if they want to formalize the concept)
...
2. Recommend running `/start` again after reviewing the artifacts for a full analysis
3. Show the recommended path:
   - `/start` — full gap analysis
   - `/design-review` — if systems index exists but GDDs are incomplete
   - `/scope-check` — validate readiness for next phase
