...
9. **Contextual next step recommendations**:
   - If the document being reviewed is `game-concept.md` or `game-pillars.md`:
     ...
     - If it does NOT exist, add to Recommendations:
       > "This concept is ready for systems decomposition. Break it down into
       > individual systems with dependencies and priorities, then write
       > per-system GDDs."
   - If the document is an individual system GDD:
     ...
     - Note: This skill is read-only. The user (or `/design-review`) must
